...
namespace Siscoo.comunicaciones
{
    ...
    {
        const string url = "http://10.0.2.2:3000/api/v1/transporte";
        ...
        public async Task<ApiResponse> Add(string token, Transporte transporte)
        {
            HttpClient cliente = await get();
            cliente.DefaultRequestHeaders.Add("Authorization", "Bearer " + token);
            var response = await cliente.PostAsync(url,
                new StringContent(JsonConvert.SerializeObject(transporte), Encoding.UTF8, "application/json"));
            ApiResponse apiResponse = new ApiResponse();
        ...
        public async Task<ApiResponse> update(string token, Transporte transporte)
        {

            HttpClient client = await get();
            client.DefaultRequestHeaders.Add("Authorization", "Bearer " + token);
            var response = await client.PutAsync(url + "/" + transporte.id, new StringContent(JsonConvert.SerializeObject(transporte), Encoding.UTF8, "application/json"));
            ApiResponse apiResponse = new ApiResponse();
            apiResponse.code = response.StatusCode.ToString();
            apiResponse.message = await response.Content.ReadAsStringAsync();
            return apiResponse;
